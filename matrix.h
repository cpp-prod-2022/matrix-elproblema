...
public:
	...
	Residue(long long n): x(((n % int(N)) + int(N)) % N) {}
...
public:
	...
	size_t find_leader(const std::vector<Field> &x) const {
		for (size_t j = 0; j < x.size(); ++j) if (x[j] != 0) return j;
		return x.size();
	}
...
private:
	...
	template<size_t K>
	void trivial_multiplication(const Matrix<N, K, Field> &to_mul) {
		vec res(M, std::vector<Field>(K));
		for (size_t i = 0; i < M; ++i) {
			for (size_t j = 0; j < N; ++j) {
				for (size_t k = 0; k < K; ++k) {
					res[i][k] += a[i][j] * to_mul[j][k];
				}
			}
		}
		a = res;
	...
	Field gauss() {
		...
		for (size_t i = 0; i < m; ++i) {
			for (size_t j = m - 1; j > i; --j) {
				if (find_leader(a[j - 1]) > find_leader(a[j])) {
					swap(a[j - 1], a[j]);
					sign ^= 1;
				}
			}
			size_t j = find_leader(a[i]);
			if (j == n) break;
...
public:
	...
	Matrix(): a(vec(M, std::vector<Field>(N, 0))) {}

	Matrix(vec mat): a(mat) {}
	...
	std::array<Field, M> getColumn(size_t j) const {
		...
		for (size_t i = 0; i < M; ++i) res[i] = a[i][j];
		return res;
	}	

	std::array<Field, N> getRow(size_t i) const {
		std::array<Field, N> res;
		for (size_t j = 0; j < N; ++j) res[j] = a[i][j];
		return res;
	}
	...
	Matrix<N, M, Field> transposed() const {
		vec trans(N, std::vector<Field>(M));
		for (size_t i = 0; i < M; ++i) {
			for (size_t j = 0; j < N; ++j) {
				trans[j][i] = a[i][j];
			}
	...
	Matrix<M, N, Field>& operator *= (Field lambda) {
		for (size_t i = 0; i < M; ++i) {
			for (size_t j = 0; j < N; ++j) {
				a[i][j] *= lambda;
			}
	...
	Matrix<M, N, Field>& operator *= (const Matrix<M, N, Field> &x) {
		static_assert(M == N);
		trivial_multiplication(x);
		return *this;
	}
	
	Matrix<M, N, Field>& operator += (Matrix<M, N, Field> b) {
		for (size_t i = 0; i < M; ++i) {
			for (size_t j = 0; j < N; ++j) {
				a[i][j] += b[i][j];
			}
	...
	friend Matrix<M_, K_, F> operator * (Matrix<M_, N_, F> A, const Matrix<N_, K_, F>& B);
};
...
template<size_t M_, size_t N_, size_t K_, typename F>
Matrix<M_, K_, F> operator * (Matrix<M_, N_, F> A, const Matrix<N_, K_, F>& B) {
	A.trivial_multiplication(B);
	return Matrix<M_, K_, F>(A.a);
}
